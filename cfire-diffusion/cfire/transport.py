...
class Transport:
    ...
    def stream_chat(
        ...
        """Return a streaming POST context for /chat/completions.

        Caller is responsible for SSE parsing and error classification.
        Compression and auth headers are applied here, mirroring post_chat().
        """
        if self._client is None:
            raise RuntimeError("Transport not opened — use `async with Transport(...)`")
